...
async function loadCrypto() {
  if (typeof window !== "undefined" && window.crypto || globalThis && globalThis.crypto) {
    const crypto2 = globalThis.crypto || window.crypto;
...
// src/cli.ts
sade(`${name} <src> <dest> [password]`, true).version(version).describe(
  "Encrypt the <src> HTML file with [password] and save the result in the <dest> HTML file."
).example("index.html encrypted.html password").example("index.html encrypted.html --generate-password 64").example("index.html encrypted.html -g 64").example("index.html encrypted.html password --iterations 3e6").example("index.html encrypted.html -g 64 --i 3e6").option(
  "-g, --generate-password",
  "Generate a random password with given length. Must be a number if used."
).option("-i, --iterations", "The number of password iterations.").action(async (src, dest, password, options) => {
